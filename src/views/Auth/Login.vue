<template>
  <div class="h-screen flex justify-center items-center relative flex">
    <div class="flex-none w-1/3 bg-login-image bg-cover bg-no-repeat h-full md:block hidden">
      <div class="pl-16">
      ...
      <div class="footer absolute bottom-10 text-primary-50 flex justify-center w-1/3 text-sm">
    ...
    <div class="bg-danger overflow-hidden flex-1">
      <div class="md:px-20 px-6 py-14 md:w-33rem mx-auto" id="modal-container">
        ...
        <div class="text-2xl font-bold mb-4 text-center">Login to your account</div>
...
function ValidateFields() {
  ...
  if (emptyField.value == false && validEmail.value == true && passWordEmpty.value == false) {
    login()
  } else {
...
const login = async () => {
  ...
  await store
    .login(values)
    .then(() => {
      openNotificationWithIcon('success', null, 'You have successfully signed in!', 3)
      router.push('/pos')
    })
